...
specs =
  Context "Contexht" [
    ...
    Context ".assert" [
      It "should return PASS if its argument is true"  $ assertEqual PASS (assert True),
      It "should return FAIL if its argument is false" $ assertEqual FAIL (assert False)
    ],

    -- Note:  yes, this looks a bit odd.
    -- I moved this here after I "bootstrapped" the tests for Contexht using HUnit, and I satisfied myself that
    -- if I changed the definition of assertEqual, all manner of other things broke.
    Context ".assertEqual" [
    ...
    let threeSpecs = Context "three specs" [context_with_one_passing_spec, context_with_one_failing_spec, pending_spec] in
    Context ".specBar" [
